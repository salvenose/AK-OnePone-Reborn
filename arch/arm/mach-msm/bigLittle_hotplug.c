...
#define MPDEC_TAG			"bigLittle_hotplug"
...
static struct cpu_hotplug {
	...
	unsigned int bigLittle_enabled;
	struct mutex bigLittle_hotplug_mutex;
	struct mutex bigLittle_cpu_mutex;
} hotplug = {
...
static int mp_decision(void) {
	...
	} else if ((nr_cpu_online > 1) && ((rq_depth < NwNs_Threshold[index]) || (rq_depth > (NwNs_Threshold[index+1])))) {
...
static void __ref bigLittle_hotplug_work(struct work_struct *work) {
	unsigned int cpu;
...
static void bigLittle_hotplug_suspend(void)
{
	int cpu;

	mutex_lock(&hotplug.bigLittle_hotplug_mutex);
	hotplug.suspended = 1;
	...
	mutex_unlock(&hotplug.bigLittle_hotplug_mutex);
	...
	pr_info(MPDEC_TAG": Screen -> off. Deactivated bigLittle hotplug. | Mask=[%d%d%d%d]\n",
			cpu_online(0), cpu_online(1), cpu_online(2), cpu_online(3));
}

static void __ref bigLittle_hotplug_resume(void)
{
	...
	if (hotplug.suspended) {
		...
		if (hotplug.max_cpus_online_susp <= 1) {
			...
			INIT_DELAYED_WORK(&hotplug_work, bigLittle_hotplug_work);
		}
	}
	...
	if (required_reschedule) {
		...
		pr_info(MPDEC_TAG": Screen -> on. Activated bigLittle hotplug. | Mask=[%d%d%d%d]\n",
				cpu_online(0), cpu_online(1), cpu_online(2), cpu_online(3));
	}
}
				...
				unsigned long event, void *data)
{
	...
	switch (event) {
		case STATE_NOTIFIER_ACTIVE:
			bigLittle_hotplug_resume();
			break;
		case STATE_NOTIFIER_SUSPEND:
			bigLittle_hotplug_suspend();
			break;
		default:
			break;
	}
...
static int bigLittle_hotplug_start(void)
{
	int cpu, ret = 0;
	struct down_lock *dl;

	hotplug_wq = alloc_workqueue("bigLittle_hotplug", WQ_HIGHPRI | WQ_FREEZABLE, 0);
	if (!hotplug_wq) {
		ret = -ENOMEM;
		goto err_out;
	}
	...
	mutex_init(&hotplug.bigLittle_cpu_mutex);
	mutex_init(&hotplug.bigLittle_hotplug_mutex);
...
err_out:
	hotplug.bigLittle_enabled = 0;
	return ret;
}

static void bigLittle_hotplug_stop(void)
{
	int cpu;
	struct down_lock *dl;
	...
	cancel_delayed_work_sync(&hotplug_work);
	mutex_destroy(&hotplug.bigLittle_hotplug_mutex);
	mutex_destroy(&hotplug.bigLittle_cpu_mutex);
#ifdef CONFIG_STATE_NOTIFIER
...
(struct device *dev, struct device_attribute *bigLittle_hotplug_attrs,	\
 char *buf)								\
{									\
...
show_one(bigLittle_enabled, bigLittle_enabled);
...
static DEVICE_ATTR(file_name, 644, show_##file_name, store_##file_name);
define_one_nwns(nwns_threshold_0_low, 0);
define_one_nwns(nwns_threshold_0_high, 1);
define_one_nwns(nwns_threshold_1_low, 2);
define_one_nwns(nwns_threshold_1_high, 3);
define_one_nwns(nwns_threshold_2_low, 4);
define_one_nwns(nwns_threshold_2_high, 5);
define_one_nwns(nwns_threshold_3_low, 6);
define_one_nwns(nwns_threshold_3_high, 7);

static ssize_t show_idle_freq (struct device *dev,
				struct device_attribute *bigLittle_hotplug_attrs,
				char *buf)
{
...
static ssize_t store_bigLittle_enabled(struct device *dev,
				struct device_attribute *bigLittle_hotplug_attrs,
				const char *buf, size_t count)
{
	unsigned int input;
	...
	if (input == hotplug.bigLittle_enabled)
		return count;
...
static DEVICE_ATTR(enabled, 644, show_bigLittle_enabled, store_bigLittle_enabled);

static struct attribute *bigLittle_hotplug_attrs[] = {
	...
	&dev_attr_nwns_threshold_0_low.attr,
	&dev_attr_nwns_threshold_0_high.attr,
	&dev_attr_nwns_threshold_1_low.attr,
	&dev_attr_nwns_threshold_1_high.attr,
	&dev_attr_nwns_threshold_2_low.attr,
	&dev_attr_nwns_threshold_2_high.attr,
	&dev_attr_nwns_threshold_3_low.attr,
	&dev_attr_nwns_threshold_3_high.attr,
	NULL,
};

static struct attribute_group attr_group = {
	.attrs = bigLittle_hotplug_attrs,
	.name = "conf",
};

/**************************** SYSFS END ****************************/

static int bigLittle_hotplug_probe(struct platform_device *pdev)
{
	int ret = 0;
	...
	ret = sysfs_create_group(bigLittle_kobj,
			&attr_group);

        if (ret) {
		pr_err("%s bigLittle_kobj create failed!\n",
			__func__);
		goto err_dev;
	}

	if (hotplug.bigLittle_enabled) {
		ret = bigLittle_hotplug_start();
		if (ret != 0)
			goto err_dev;
	}
	...
	return ret;
err_dev:
	if (bigLittle_kobj != NULL)
		kobject_put(bigLittle_kobj);
	return ret;
}

static struct platform_device bigLittle_hotplug_device = {
	.name = MPDEC_TAG,
	.id = -1,
};

static int bigLittle_hotplug_remove(struct platform_device *pdev)
{
	if (hotplug.bigLittle_enabled)
		bigLittle_hotplug_stop();
...
static struct platform_driver bigLittle_hotplug_driver = {
	.probe = bigLittle_hotplug_probe,
	.remove = bigLittle_hotplug_remove,
	.driver = {
		.name = MPDEC_TAG,
		.owner = THIS_MODULE,
	},
...
static int __init biglittle_mpdec_init(void)
{
	...
	ret = platform_driver_register(&bigLittle_hotplug_driver);
	if (ret) {
		pr_err("%s: Driver register failed: %d\n", MPDEC_TAG, ret);
		return ret;
	}

	ret = platform_device_register(&bigLittle_hotplug_device);
	if (ret) {
